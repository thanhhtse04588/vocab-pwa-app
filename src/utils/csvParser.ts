...
export const parseCSV = (csvContent: string): CSVParseResult => {
  ...
  try {
    ...
    const meaningIndex = findColumnIndex(headers, [
      'meaning',
      'definition',
      'translation',
    ]);
    const pronunciationIndex = findColumnIndex(headers, [
      'pronunciation',
      'phonetic',
      'sound',
    ]);
    const exampleIndex = findColumnIndex(headers, [
      'example',
      'sentence',
      'usage',
    ]);
    const wordTypeIndex = findColumnIndex(headers, [
      'wordtype',
      'word_type',
      'type',
      'part_of_speech',
      ...
      try {
        ...
        const pronunciation =
          pronunciationIndex !== -1
            ? values[pronunciationIndex]?.trim()
            : undefined;
        const example =
          exampleIndex !== -1 ? values[exampleIndex]?.trim() : undefined;
        const wordType =
          wordTypeIndex !== -1 ? values[wordTypeIndex]?.trim() : undefined;
        ...
        result.data.push({
          ...
          wordType: (wordType as any) || undefined,
        });
      } catch (error) {
...
export const downloadCSVTemplate = (): void => {
  const template =
    'word,meaning,pronunciation,example,wordtype\n"hello","greeting","/həˈloʊ/","Hello, how are you?","interjection"\n"goodbye","farewell","/ɡʊdˈbaɪ/","Goodbye, see you later!","interjection"\n"book","a written work","/bʊk/","I read a book.","noun"';
